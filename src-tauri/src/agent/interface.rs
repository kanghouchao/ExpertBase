...
fn load_settings(app: &tauri::AppHandle) -> Result<AiSettings, AppError> {
...
#[tauri::command]
pub fn ai_has_key(app: tauri::AppHandle) -> Result<bool, AppError> {
...
#[tauri::command]
pub fn ai_list_ollama_models(app: tauri::AppHandle) -> Result<Vec<ollama::OllamaModel>, AppError> {
  let settings = load_settings(&app)?;
  let base = resolve_base_url(Provider::Ollama, &settings.ollama_url);
  ollama::list_models(&base)
}

/// 指定 provider + URL のモデル一覧（設定画面の「検証」用。URL は未保存の入力値をそのまま受ける）。
/// 空欄なら provider 既定へ解決。成功＝端点が生きていて列挙できた＝検証 OK。
#[tauri::command]
pub fn ai_list_models(provider: Provider, base_url: String) -> Result<Vec<ollama::OllamaModel>, AppError> {
  let base = resolve_base_url(provider, &base_url);
  match provider {
    Provider::Ollama => ollama::list_models(&base),
    Provider::LlamaApp => openai_compat::list_models(&base),
  }
}

/// 保存済みの AI 設定を読む（無ければ既定値）。
#[tauri::command]
pub fn ai_get_settings(app: tauri::AppHandle) -> Result<AiSettings, AppError> {
  load_settings(&app)
}

/// AI 設定を保存する。
#[tauri::command]
pub fn ai_set_settings(app: tauri::AppHandle, settings: AiSettings) -> Result<(), AppError> {
  let home = app.path().home_dir().map_err(AppError::generic)?;
