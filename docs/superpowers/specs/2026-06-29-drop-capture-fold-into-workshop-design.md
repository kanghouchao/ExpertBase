...
- ただし `entry.sources` には、同じ agent 実行内で `read_source` / `fetch_web` が正常に読んだ**引用文字列（外部パス / URL）を残す**。添付しただけで読まなかった素材は記録しない。リンクは原ファイルの移動/削除で烂れ得るが、知識庫は「これは `/…/report.pdf` 由来」を知っておくべき。
