...
- **`workshop_chat` 経由の生成は現状 tools 対応モデル必須**（`workshop-session.ts` の `canGenerate = ... && selectedTools && ...`）。tools 非対応モデルは現行 UI では生成ボタン自体が押せない。この事実は「仮定/要検証」で扱う（後述）。
...
- **前端の `canGenerate` ゲートと「tools 非対応モデルでも明示発動は使える」という受け入れ条件の間に実践上のギャップがある**: `workshop-session.ts` は現在 `selectedTools` を必須にして生成ボタンを塞ぐため（"工坊は tools 対応モデル必須"）、tools 非対応モデルは今の UI では `workshop_chat` へ到達できない。したがって本設計の `tools_capable=false` 分岐（catalog なし・`activate_skill` 未登録・`# Activated Skills` のみ有効）は**バックエンドの単体テストでは検証できるが、v1 の実 UI では到達不能な経路**になる。これは #41 単体の範囲を超える製品判断（`canGenerate` を緩めて「tools 非対応でも KB ツール無しで発動済み技能だけの対話は許す」かどうか）であり、実装フェーズで issue owner と要確認。今回の実装では**バックエンド側は正しく両分岐を実装しコードレベルで守る**が、`canGenerate` 自体の変更はスコープ外として着手しない。
